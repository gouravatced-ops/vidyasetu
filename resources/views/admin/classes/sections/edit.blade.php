        ...
        <div class="panel-body">
            <div class="card rounded-4 border-0 card-modern-form">
                <div class="card-body p-4">
                    <form action="{{ route('admin.classes.sections.update', [$schoolClass, $section]) }}" method="POST" class="class-create-form form-modern-grid">
                        @csrf
                        @method('PUT')
                        <div class="row row-cols-1 row-cols-md-2 g-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $section->name) }}" class="form-control-modern @error('name') is-invalid @enderror" required>
                                @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div>
                                <label for="display_name" class="form-label-modern">Display Name</label>
                                <input type="text" name="display_name" id="display_name" value="{{ old('display_name', $section->display_name) }}" class="form-control-modern @error('display_name') is-invalid @enderror">
                            ...
                            <div class="col-12 form-create-actions">
                                ...
                                <button type="submit" class="btn-primary-modern"><i class="fas fa-save me-2"></i>Update Section</button>
                            </div>
                        </div>
                    </form>
                </div>
